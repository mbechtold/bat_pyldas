...
def plot_filter_diagnostics(exp, domain, root, outpath):
    ...
    figure_quatro_default(data_all, lons=lons, lats=lats, cmin=cmin, cmax=cmax, llcrnrlat=llcrnrlat,
                          ...
                          llcrnrlon=llcrnrlon, urcrnrlon=urcrnrlon, outpath=outpath, exp=exp, fname=fname,
                          plot_title=(
                          ['n_valid_innov_H_Asc', 'n_valid_innov_H_Des', 'n_valid_innov_V_Asc', 'n_valid_innov_V_Des']))
    ...
    figure_quatro_default(data=data_all, lons=lons, lats=lats, cmin=cmin, cmax=cmax, llcrnrlat=llcrnrlat,
                          ...
                          llcrnrlon=llcrnrlon, urcrnrlon=urcrnrlon, outpath=outpath, exp=exp, fname=fname,
                          plot_title=(
                          ['norm_innov_mean_H_Asc [K]', 'norm_innov_mean_H_Des [K]', 'norm_innov_mean_V_Asc [K]',
...
def plot_innov_quatro(exp, domain, root, outpath, dti):
    ...
    for timestamp in timestamps:
        ...
        figure_quatro_default(data, lons=lons, lats=lats, cmin=cmin, cmax=cmax, llcrnrlat=llcrnrlat,
                              urcrnrlat=urcrnrlat,
                              llcrnrlon=llcrnrlon, urcrnrlon=urcrnrlon, outpath=outpath, exp=exp, fname=fname,
                              plot_title=list(['O-F (Tb in K) H-Asc', 'O-F (Tb in K) H-Des', 'O-F (Tb in K) V-Asc',
                                               'O-F (Tb in K) V-Des']))
...
def plot_timeseries_RSEpaper(exp1, exp2, domain, root, outpath, lat=53, lon=25):
    ...
    ax1 = plt.subplot(811)
    df1[['sfmc analysis (CLSM)', 'sfmc open loop (PCLSM)']].plot(ax=ax1, fontsize=fontsize, style=['-', '-'],
                                                                 color=[col_CLSM, col_PCLSM], linewidth=2)
    plt.ylabel('sfmc [-]')

    ax2 = plt.subplot(812)
    df1[['tp1 analysis (CLSM)', 'tp1 open loop (PCLSM)']].plot(ax=ax2, fontsize=fontsize, style=['-', '-'],
                                                               color=[col_CLSM, col_PCLSM], linewidth=2)
    plt.ylabel('tp1 [-]')

    ax3 = plt.subplot(813)
    df[['Tb obs (species=1)', 'Tb fcst CLSM OL (species=1)', 'Tb fcst PCLSM OL (species=1)']].plot(ax=ax3,
                                                                                                   fontsize=fontsize,
                                                                                                   style=['.-', '.-',
                                                                                                          '.-'],
                                                                                                   color=['k', col_CLSM,
    ...
    ax4 = plt.subplot(814)
    df[['Tb obs (species=3)', 'Tb fcst CLSM OL (species=3)', 'Tb fcst PCLSM OL (species=3)']].plot(ax=ax4,
                                                                                                   fontsize=fontsize,
                                                                                                   style=['.-', '.-',
                                                                                                          '.-'],
                                                                                                   color=['k', col_CLSM,
    ...
    ax5 = plt.subplot(815)
    df[['Tb obs resc. CLSM (species=1)', 'Tb fcst CLSM OL (species=1)']].plot(ax=ax5, fontsize=fontsize,
                                                                              style=['.-', '.-'], color=['k', col_CLSM],
                                                                              linewidth=2)
    plt.ylabel('Tb [K]')

    ax6 = plt.subplot(816)
    df[['Tb obs resc. CLSM (species=3)', 'Tb fcst CLSM OL (species=3)']].plot(ax=ax6, fontsize=fontsize,
                                                                              style=['.-', '.-'], color=['k', col_CLSM],
                                                                              linewidth=2)
    plt.ylabel('Tb [K]')

    ax7 = plt.subplot(817)
    df[['Tb obs resc. PCLSM (species=1)', 'Tb fcst PCLSM OL (species=1)']].plot(ax=ax7, fontsize=fontsize,
                                                                                style=['.-', '.-'],
                                                                                color=['k', col_PCLSM], linewidth=2)
    plt.ylabel('Tb [K]')

    ax8 = plt.subplot(818)
    df[['Tb obs resc. PCLSM (species=3)', 'Tb fcst PCLSM OL (species=3)']].plot(ax=ax8, fontsize=fontsize,
                                                                                style=['.-', '.-'],
                                                                                color=['k', col_PCLSM], linewidth=2)
    plt.ylabel('Tb [K]')
